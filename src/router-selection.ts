 ...
 * when no selector extension is in play.
 *
 * Backed by a temp file, not a plain module-level variable: Pi isolates each extension's module
 * graph, so `notdiamond-router.ts` and `call-logger.ts` each get their own separate instance of
 * this module even though both resolve the identical file path — a file is genuinely process-wide
 * regardless of that isolation.
 *
 * Safe under this project's own concurrency model: `runner.ts` runs one `pi` process at a time,
 * strictly sequentially, and `scripts/run-comparison.ts` runs its arms sequentially too — so
